...
**Concepts:** token bucket / sliding window algorithms, distributed counters, Redis atomicity (Lua scripts), latency-vs-accuracy trade, API gateway placement, 429 semantics, graceful degradation.

**The hard part they're probing:** enforcing one global limit across N stateless API servers without adding a synchronous Redis round trip to every single request — and knowing what happens when the limiter's own datastore is down.
...
**Out of scope:** DDoS mitigation at L3/L4 (that's upstream, at the CDN/scrubbing layer), billing.
...
**Why the Lua script matters.** `GET` then `SET` is a read-modify-write race: two gateways both read 5 tokens, both decrement, both write 4, and two requests consumed one token. Redis executes Lua scripts atomically on its single thread, making check-and-decrement one indivisible operation and one round trip.
...
**Sharding Redis.** Hash by identity, so all state for one key lives on one node and the Lua script is a single-node atomic operation. Cross-slot operations would break atomicity — a good detail.

**Hot keys.** One enormous customer's key concentrates on one Redis shard. Either give large tenants dedicated shards, or shard their key into sub-buckets (`key#0..#9`, each with 1/10 the limit) and pick one at random per request.

**What to limit by, and the trap.** Per user ID is fairest but requires authentication, so the *authentication* endpoint itself must be limited by IP. IP limiting is blunt: corporate NAT means thousands of users share an IP, and attackers rotate proxies. The realistic answer is layered limits — IP at the edge, API key at the gateway, user ID at the service, plus per-endpoint overrides for expensive operations.

---

## Possible follow-up questions

- *A client ignores 429 and hammers you.* Escalate: exponentially longer cooldowns, then temporary blocks at the CDN/WAF layer so the traffic never reaches your gateways.
- *How do you support "1,000 requests per day" alongside "10 per second"?* Multiple buckets checked in sequence; deny if any denies. Return the most restrictive `Retry-After`.
- *Rate limiting by cost, not count?* Assign a weight per endpoint and deduct that many tokens. Natural extension of token bucket, and how most real API quota systems work.
- *How do you roll out a limit change safely?* Run in shadow mode first — evaluate the rule and log what *would* have been rejected, without rejecting. Then enable. This prevents accidentally cutting off a major customer.
- *What do you monitor?* Rejection rate per tier (a spike means either an attack or a misconfigured limit), Redis latency p99, sync lag, and overshoot magnitude.
- *Client-side rate limiting?* Useful and cheap (respect `Retry-After`, self-throttle) but never a substitute — you can't trust the client.
